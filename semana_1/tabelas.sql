...
INSERT INTO tbEstado (Nome, UF, IdPais) VALUES
('Acre', 'AC', 1),
('Alagoas', 'AL', 1),
('Amazonas', 'AM', 1),
('Amapá', 'AP', 1),
...
INSERT INTO tbCidade (Nome, IdEstado) VALUES
('Afonso Cláudio', 8),
...
/* Tipo text, datetime, decimal float, armazenar arquivo */

CREATE TABLE tbPlano (
	IdPlano int identity(1,1) not null primary key,
	Nome varchar(100) not null,
	Validade int not null, /* Meses */
...
/******************************************************************************/
/*** Ver a opção de uma Tabela Vendedor ***/
/******************************************************************************/

CREATE TABLE tbTipoAcesso (
	IdTipoAcesso int identity(1,1) not null primary key,
	Nome varchar(100) not null,
...
CREATE TABLE tbTipoProfissional ( /* Nutricionista, Médico, etc */
	IdTipoProfissional int identity(1,1) not null primary key,
	Nome varchar(100)
)

CREATE TABLE tbProfissional (
	IdProfissional int identity(1,1) not null primary key,
	IdTipoProfissional int,
	...
	foreign key (IdTipoAcesso) references tbTipoAcesso (IdTipoAcesso),
	foreign key (IdCidade) references tbCidade (IdCidade),
)

/******************************************************************************/

/*** GrupoPatologico | NivelPatologico | Patologia ***/


CREATE TABLE tbGrupoPatologico ( /* Área da Patologia? */
	IdGrupoPatologico int identity(1,1) not null primary key,
	Nome varchar(100) not null,
)

CREATE TABLE tbPatologia (
	IdPatologia int identity(1,1) not null primary key,
	Nome varchar(100) not null,
	InformacaoComplementar varchar(5000),
)

CREATE TABLE tbGrupoPatologico_X_Patologia (
	IdPatologia_X_GrupoPatologico int identity(1,1) not null primary key,
	IdGrupoPatologico int not null, /* Associe pelo menos uma Patologia ao grupo */
	IdPatologia int not null,
	foreign key (IdGrupoPatologico) references tbGrupoPatologico (IdGrupoPatologico),
	foreign key (IdPatologia) references tbPatologia (IdPatologia)
)

/******************************************************************************/

/*** UnidadePeso | Quantidade | Substancia | TipoMedicamento | CategoriaMedicamento | Medicamento ***/

CREATE TABLE tbSubstancia ( /* Auto-Completar */
	IdSubstancia int identity(1,1) not null primary key,
	Nome varchar(100) not null,
	InformacaoComplementar varchar(5000)
)

CREATE TABLE tbCategoriaMedicamento (
	IdCategoriaMedicamento int identity(1,1) not null primary key,
	Nome varchar(100) not null,
	InformacaoComplementar varchar(5000)
)
...
CREATE TABLE tbMedicamento (
	...
	IdCategoriaMedicamento int not null, /* Na mesma tela, Ex: Essa Catergoria não existe, deseja adicioná-la? */
	Nome varchar(100) not null,
	Bula varchar(MAX), /*** ou um ou outro ***/
	BulaArquivo varbinary(MAX),
	FOREIGN KEY (IdCategoriaMedicamento) REFERENCES tbCategoriaMedicamento(IdCategoriaMedicamento),
)

/******************************************************************************/

/*** Alimento | ReceitaAlimentar | QuantidadeAlimento | Suplemento***/

/* Ver porcentagem baseado em mulher, criança, adulto, etc, ... */

CREATE TABLE tbAlimento ( /* Tem a listagem de alimentos padrao + o cadastrado pelo próprio profissional*/
	IdAlimento int identity(1,1) not null primary key,
	IdTipoQuantidade int not null,
	Nome varchar(100) not null,
	Carboidrato float not null, /* Inicializar tudo com Zero - Ver unidade Mais comum*/
	VitaminaA float not null,
	VitaminaB float not null,

)

CREATE TABLE tbSuplemento (  /* Tem a listagem de suplemento padrao + o cadastrado pelo próprio profissional*/
	IdSuplemento int identity(1,1) not null primary key,
	IdTipoQuantidade int not null,
	Tipo Int not null,
	Nome varchar(100),
	DoseMinima float not null, /* Perguntar */
...
CREATE TABLE tbReceitaAlimentarPadrao ( 	IdReceitaAlimentarPadrao int identity(1,1) not null primary key,
	IdProfissional int,
	Nome varchar(100) not null,
	InformacaoComplementar varchar(5000), /*Prescrição lá embaixo */
	foreign key (IdProfissional) references tbProfissional (IdProfissional),
) /* Uma receita alimentar padronizada */

CREATE TABLE tbReceitaMedicaPadrao ( /* Ter a opção de compartinlhar com os outros profissionais da clínica ou Empresa */
	IdReceitaMedicaPadrao int identity(1,1) not null primary key,
	IdProfissional int not null,
	Nome varchar(50) not null,
	InformacaoComplementar varchar(5000), /*Prescrição lá embaixo */
	foreign key (IdProfissional) references tbProfissional (IdProfissional),
) /* Uma receita médica padronizada */

CREATE TABLE tbReceitaAlimentarPadrao_X_Alimento (
	IdReceitaAlimentarPadrao_X_Alimento_X_QuantidadeAlimento int identity(1,1) not null primary key,
	IdReceitaAlimentarPadrao int not null, /* Consigo pegar o profissional aqui */
	IdAlimento int not null,
	IdQuantidadeAlimento int not null,
	Periodicidade varchar(100),
	QuantoTempo varchar(100),
	foreign key (IdReceitaAlimentarPadrao) references tbReceitaAlimentarPadrao (IdReceitaAlimentarPadrao),
...
/***  Paciente | Medico_Paciente | Ferias | DiaAtende | HoraAtende | HoraPaciente | HoraProfissional ***/
...
CREATE TABLE tbPaciente (
	...
	NomeResponsavel varchar(100), /* Data de nascimento < 18 obrigatório, senão, marcar a caixa para aparecer */
	Sexo char not null,
	Etnia int not null,
	Endereco varchar(100),
	Bairro varchar(50),
	IdCidade int,
	TelResidencial varchar(25),
	TelComercial varchar(25),
	TelCelular varchar(25),
...
CREATE TABLE tbMedico_Paciente (
	IdMedico_Paciente int identity(1,1) not null primary key,
	IdPaciente int not null,
	IdProfissional int not null,
	InformacaoResumida varchar(5000), /*** Cada médico pode ter informações gerais sobre cada paciente ***/
	FOREIGN KEY (IdPaciente) REFERENCES tbPaciente(IdPaciente),
	FOREIGN KEY (IdProfissional) REFERENCES tbProfissional(IdProfissional)
)

CREATE TABLE tbHoraPaciente_Profissional ( 	IdHoraPaciente_Profissional int identity(1,1) not null primary key,
	IdPaciente int not null,
	IdProfissional int not null,
	DataConsulta date,
...
/***  HistoricoDoencaAtual | HistoriaPatologia***/

CREATE TABLE tbHistoricoDoencaAtual (
	IdHDA int identity(1,1) not null primary key,
	IdPaciente int not null,
	IdPatologia int, /* Mais de uma provavelmente */
	...
	Metastase int,
	Queimadura int,
	ObsNeoplasia varchar(250),
	ObsMetastase varchar(250),
	ObsQueimadura varchar(250),
	Outros varchar(250),
	FOREIGN KEY (IdPaciente) REFERENCES tbPaciente(IdPaciente),
	FOREIGN KEY (IdPatologia) REFERENCES tbPatologia(IdPatologia)
)

CREATE TABLE tbHistoriaPatologica (
	IdHistoriaPatologica int identity(1,1) not null primary key,
	IdPaciente int not null,
	IdPatologia int,
	FlgHAS bit,
	FlgAVC bit,
	FlgDoencasPulmonares bit,
	FlgDoencasCardiacas bit,
	FlgDoencaRenal bit,
	FlgDoencaHepatica bit,
	FlgNeoplasia bit,
	FlgHipercolesterolemia bit,
	FlgHipertrigliciridemia bit,
	FlgHiperuricemia bit,
	FlgAnemias bit,
	FlgCirurgias bit,
	FlgDoencasAutoImunes bit,
	FlgDiabetes bit,
	Obs varchar(500),
	FOREIGN KEY (IdPaciente) REFERENCES tbPaciente(IdPaciente),
	FOREIGN KEY (IdPatologia) REFERENCES tbPatologia(IdPatologia)
...
CREATE TABLE tbExameFisico (
	IdExameFisico int identity(1,1) not null primary key,
	IdHoraPaciente_Profissional int, /* Pego atumaticamente da data da consulta */
	SNC int,
	AtividadeFisica int,
	TipoAtividadeFisica varchar(1000),
	Frequencia int,
	HoraPreferido int,
	Tempo int,
	FlgDenticaoCompleta bit,
	FlgDenticaoIncompleta bit,
	FlgDenticaoAusente bit,
	FlgDenticaoProtese bit,
	Mastigacao int,
	Peristalse int,
	Fumante int,
	...
	FlgOral bit,
	FlgOralEnteral bit,
	FlgEnteralExclusiva bit,
	FlgEnteralParental bit,
	FlgParentalExclusiva bit,
	FlgParentalOral bit,
	TipoAcesso int,
	FlgGastrico bit,
	FlgTranspilorica bit,
	FlgDuodenal bit,
	FlgJejunal bit,
	FlgHiperemia bit,
	FlgSaidaSecrecao bit,
	FlgPresencaFeridas bit,
	Obs varchar(500),
	FOREIGN KEY (IdHoraPaciente_Profissional) REFERENCES tbHoraPaciente_Profissional(IdHoraPaciente_Profissional)
)

/******************************************************************************/

CREATE TABLE tbHistoricoSocialAlimentar (
	IdHistSocialAlimentar int identity(1,1) not null primary key,
	IdPaciente int not null,
	Profissao varchar(100),
	CargaHoraria int,
	NroPessoasRes int,
	RendaFamiliar decimal(12,2),
	Escolaridade varchar(50),
	EstadoCivil varchar(30),
	NomeCompraAlimento varchar(100),
	NomeCozinhaAlimento varchar(100),
	CompraFeita int,
	NomeRealizaRefeicao varchar(100),
	FlgTabagismo bit,
	QtdTabagismoDia int,
	FlgEtilismo bit,
	QtdEtilismoDia int,
	...
	FlgAvosHas bit,
	FlgIrmaosHas bit,
	FlgPaiMaeDiabetes bit,
	FlgAvosDiabetes bit,
	FlgIrmaosDiabetes bit,
	FlgPaiMaeCancer bit,
	FlgAvosCancer bit,
	FlgIrmaosCancer bit,
	FlgPaiMaeObesidade bit,
	FlgAvosObesidade bit,
	FlgIrmaosObesidade bit,
	FOREIGN KEY (IdPaciente) REFERENCES tbPaciente(IdPaciente),
)

CREATE TABLE tbHistoricoAlimentarNutricional (
	IdHistAlimentarNutricional int identity(1,1) not null primary key,
	IdPaciente int not null,
	MotivacaoTratamento int,
	ModismosAlimentares int,
	FlgIntoleanciaAlimentar bit,
	DescIntoleranciaAlimentar varchar(250),
	FaseObesidadePerdaPeso int,
	FlgPerdaGanhoPeso bit,
	PesoMax int,
	PesoMaxIdade int,
	PesoMin int,
	PesoMinIdade int,
	FlgDietas bit,
	DescDietas varchar(250),
	FlgMedicamentos bit,
	DescMedicamentos varchar(250),
	FlgExercicios bit,
	DescExercicios varchar(250),
	FlgOutros bit,
	DescOutros varchar(250),
	Obs varchar(500),
	FOREIGN KEY (IdPaciente) REFERENCES tbPaciente(IdPaciente)
)

CREATE TABLE tbAntropometria (
	IdAntropometria int identity(1,1) not null primary key,
...
CREATE TABLE tbExame ( 
	IdExame int identity(1,1) not null primary key,
	Grupo int not null,
	Nome varchar(100) not null,
	Descricao varchar(250)
)

CREATE TABLE tbExame_X_Pacientes (
	IdExame_X_Paciente int identity(1,1) not null primary key,
...
/* Pergunta | RastreamentoResposta | RastreamentoMetabolico */

CREATE TABLE tbPergunta ( /* Verificar generalidade ou para cada médico as perguntas */
	IdPergunta int identity(1,1) not null primary key,
	IdProfissional int not null,
...
CREATE TABLE tbRastreamentoResposta (
	IdRastreamentoResposta int identity(1,1) not null primary key,
	IdPergunta int not null,
	VlrRespota int not null,
	IdParteCorpo int not null,
	Obs varchar(500) not null,
	FOREIGN KEY (IdPergunta) REFERENCES tbPergunta(IdPergunta)
)

CREATE TABLE tbRastreamentoMetabolico (
	IdRastreamentoMetabolico int identity(1,1) not null primary key,
	IdRastreamentoResposta int not null,
	IdHoraPaciente_Profissional int not null,
	ObsGeral varchar(1000),
	Total int,
	FOREIGN KEY (IdRastreamentoResposta) REFERENCES tbRastreamentoResposta(IdRastreamentoResposta),
	FOREIGN KEY (IdHoraPaciente_Profissional) REFERENCES tbHoraPaciente_Profissional(IdHoraPaciente_Profissional)
)
...
CREATE TABLE tbEscalaBristol (
	IdEscalaBristol int identity(1,1) not null primary key,
	Nome varchar(50) not null,
	Sangue bit,
	/* Imagem varbinary(MAX), - Colocar direto na página através de um case */
)

CREATE TABLE tbEscalaBristol_Paciente_Consulta (
	IdEscalaBristol_Paciente_Consulta int identity(1,1) not null primary key,
	IdEscalaBristol int not null,
	IdPaciente int not null,
	IdHora_Paciente_Profissional int not null,
	FOREIGN KEY (IdEscalaBristol) REFERENCES tbEscalaBristol(IdEscalaBristol),
	FOREIGN KEY (IdPaciente) REFERENCES tbPaciente(IdPaciente),
	FOREIGN KEY (IdHora_Paciente_Profissional) REFERENCES tbHoraPaciente_Profissional(IdHoraPaciente_Profissional)
)
...
CREATE TABLE tbGruposReceitasDespesas (
	IdReceitaDespesa int identity(1,1) not null primary key,
	Tipo int not null,
	Nome varchar(100) not null
)

CREATE TABLE tbLancarReceitasDespesas (
	IdLancamento int identity(1,1) not null primary key,
	IdReceitaDespesa int not null,
	Data datetime not null,
	FOREIGN KEY (IdReceitaDespesa) REFERENCES tbGruposReceitasDespesas(IdReceitaDespesa)
)
